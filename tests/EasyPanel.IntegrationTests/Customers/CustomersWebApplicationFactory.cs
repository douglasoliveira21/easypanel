...
/// <c>/api/v1/customers</c> (Task 7.2 / R8). Substitui a persistência real por
/// SQLite in-memory, emite tokens reais e semeia:
/// <list type="bullet">
///   <item>um Administrador no Tenant A (possui <c>customer.view/create/edit</c>);</item>
///   <item>um Operacional no Tenant A (possui <c>customer.view/create/edit</c>,
///     porém <b>não</b> <c>user.manage</c> — usado para testar a ausência de
///     permissões de leitura via um papel sem <c>customer.view</c>);</item>
///   <item>um Técnico no Tenant A (somente leitura de clientes — sem create/edit);</item>
///   <item>um Administrador no Tenant B, para provar o isolamento por tenant (R8.8).</item>
/// </list>
/// </summary>
public sealed class CustomersWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    /// <summary>Administrador do Tenant A: possui customer.view/create/edit.</summary>
    public const string AdminAEmail = "admin@tenant-a.example.com";

    /// <summary>Técnico do Tenant A: possui apenas customer.view (sem create/edit).</summary>
    public const string TecnicoAEmail = "tecnico@tenant-a.example.com";

    /// <summary>Cliente do Tenant A: sem permissões administrativas (sem customer.view).</summary>
    public const string ClienteAEmail = "cliente@tenant-a.example.com";

    /// <summary>Administrador do Tenant B: possui customer.* no seu tenant.</summary>
    ...
    /// <summary>Abre um escopo de DI para inspeção direta do banco nos testes.</summary>
